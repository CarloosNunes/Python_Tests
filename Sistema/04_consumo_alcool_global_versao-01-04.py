...
arquivo_consumo_alcool = "D:/Clarify/Carlos Nunes/PythonAvançado/consumo_alcool.db"
arquivo_drinks = "D:/Clarify/Carlos Nunes/PythonAvançado/drinks.csv"

#configura o plotly para abrir os arquivos no navegador por padrão
pio.renderers.default = "browser"

#carregar o drinks.csv
df = pd.read_csv(arquivo_drinks)
#df_avengers = pd.read_csv("avengers.csv")

#cria o banco de dados em sql e popular com os dados do arquivo csv
conn = sqlite3.connect(arquivo_consumo_alcool)
...
def apagar_avengers():
    ...
    try:
        ...
        mensafgem = f"<h3>Erro ao apagar a tabela: {str(errinho)}</h3>"
...
###CRIAR UMA ROTA CHAMADA CONSULTAR VINGADORES, NELA QUERO MOSTRAR A TABELA VINGADORES, MAS CASO ELA NÃO EXISTA MOSTRE
#MENSAGEM DIZENDO QUE ELA NÃO EXISTE;
# ESSA ROTA NÃO EXISTE UM LINK EM NOSSO MENU INICIAL E DEVE SER CRIADO
...
def ver_avengers():
    ...
    try:
        ...
        conn.close()
        return f"<h3>Erro ao consultar a tabela:{str(e)}</h3><br>Volte e insira o arquivo<br><a href='/>Voltar ao inicio</a>"
    conn.close()
...
def consultar_avenger():
    conn = sqlite3.connect(arquivo_consumo_alcool)
    df_avengers = pd.read_sql_query("SELEC * FROM avengers", conn)
    conn.close()
